...
function newBase(seedCharacter){
    ...
    var residents = Math.floor(Math.random() * 40) + 10
...
function myFunction(){
    ...
    // chart.data(liveGraph)
    document.getElementById("stats").textContent = "Total Pop: " + totalPop+" Factions:"+ countUnique(labelsOut.colors)+" Worlds: "+labelsOut.labels.length+ " Steps: "+step
}
var refreshIntervalId = null;

document.getElementById("continue").addEventListener("click",()=>{
    refreshIntervalId = setInterval(myFunction, 1000);


})

// d="M 224.10304092638617 168.8078758880014 A 7.5 7.5 0 0 1 209.10304092638617 168.8078758880014 A 7.5 7.5 0 0 1 224.10304092638617 168.8078758880014"
// d="M 169.4452782293577 171.7516093400829 A 7.5 7.5 0 0 1 154.4452782293577 171.7516093400829 A 7.5 7.5 0 0 1 169.4452782293577 171.7516093400829"
...
// chart.data(liveGraph)
document.getElementById("stats").textContent = "Total Pop: " + totalPop+" Factions:"+ countUnique(labelsOut.colors)+" Worlds: "+labelsOut.labels.length
places.map(e=>{

    console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"+e.name+" "+e.faction+" "+e.residents.length)
})
...
document.getElementById("output").textContent = JSON.stringify(places, null, 2)
...
function genGraph(){
    ...
    places.forEach(e=>{
        ...
        liveGraph.nodes.push({
            id: e.name,
            description: e.residents.length,
        height: 15,
           color: e.color,
        fill: e.color,
        x: Math.floor(Math.random() * 100),
    ...
    console.log(liveGraph)
}

function draw(){
    
        // create a chart from the loaded data
        var chart = anychart.graph(liveGraph)

        // set the title
        chart.title("Network Graph showing Galactic Expansion");
        ...
        nodes.normal().fill("{%color}");
        nodes.hovered().fill("white");
        nodes.selected().fill("{%color}");
        ...
        chart.nodes().labels().format("{%id} Pop: {%description}");
